...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            fg_image_features = self.encode_images(images[0])
            bg_image_features = self.encode_images(images[1])
            image_features = torch.cat((fg_image_features,bg_image_features), dim=1)
