...
#img_folder = "/home/jovyan/data/xjy/ZOPA_dataset"

eval_data_path = "/home/lm3/data/new_OPA/trans_labels_gt_eval_set.json"
#eval_data_path = "/home/jovyan/data/xjy/ZOPA_dataset/trans_labels_gt_eval_set.json"
...
def eval_model(args):
    ...
    if args.token_idx is None:
        save_dir = os.path.join(save_dir, "pred_max")
    else:
        save_dir = os.path.join(save_dir, f"pred{args.token_idx}")
    eval_dir = os.path.join(save_dir, args.eval_type, str(args.epoch))
    ...
    with open(eval_data_path, "r") as f:
        ...
        for index,data_info in tqdm(enumerate(eval_data_infos)):
            ...
            masked_fg_img_tensor = image_processor.preprocess(masked_fg_img, return_tensors='pt')['pixel_values'].half().cuda()    
            bg_img_tensor = image_processor.preprocess(bg_img, return_tensors='pt')['pixel_values'].half().cuda()
            ...
            with torch.inference_mode():
                pred_trans = model.generate_trans_label(
                    input_ids=input_ids,
            ...
            for repeat_id in range(args.repeat):
                gen_comp_img, gen_comp_msk, gen_comp_bbox = gen_composite_image(
                    ...
                    trans=(pred_trans.cpu().numpy().astype(np.float32)[0]).tolist(),
                    fg_bbox=None
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", type=str, default="/home/lm3/projects/LLaVA_cvpr2024/rebuttal_res")
